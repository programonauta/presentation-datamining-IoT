	...
	\frametitle{Método}
     \begin{itemize}
            ...
            \item Aprofundamento dos conceitos básicos \begin{itemize}
	            \item Internet das Coisas
  	         	\item Mineração de dados \begin{itemize}
		            	\item Clusterização
		            	\item Sumarização de dados
		            	\item Detecção de desvios
		            	\item Mineracao de dados distribuída
...
\begin{frame}
	...
	Exemplo de representação em três abordagens: \begin{itemize}
		\item Geométrica (esférica -- centroide e raio, não-convexa, etc)
		\item Estatística (frequência)
		\item Semântica (predicados)
	\end{itemize}
...
\begin{frame}
       ...
       Detecção de desivos de comportamento \begin{itemize}
               \item \cite{022-000} (Qualis A1) propõe uma nova formulação na detecção de \textit{outliers} com base na distância utilizando um algoritmo baseado em partições
               \item \cite{023-000} (Qualis B2) utiliza o algoritmo LOF (\it{Local Outlier Factor}) aplicado em um grid com o objetivo de reduzir o tempo de computação na descoberta de desvio de comportamentos.
       \end {itemize}
\end{frame}
...
%            \item Adequação / Desenvolvimento de algoritmo no contexto da internet das coisas \begin{itemize}            		
...
%	\frametitle{Visão Geral -- Identificação de Desvios}
%			
%	Análise dos algorítmos de Identificação de Desvios \begin{itemize}
%		\item Algoritmo Baseado em Partições \cite{022-000}
%		\item Algoritimo LOF (\textit{Local Outlier Factor}) aplicado em grid \cite{023-000}
%		\item Adequação / Desenvolvimento de algoritmo no contexto da internet das coisas \begin{itemize}            		
%		       \item Poucos recursos computacionais
%		                    \item Redução no tráfego de informações
%		            \end{itemize}
